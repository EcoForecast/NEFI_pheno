...
createEmptyFiles <- function(siteData,day,year){
  for(i in 1:length(siteData)){
    siteName <- as.character(siteData[i,1])
    fileName <- paste("GOES_NDVI_Leftover3",siteName,"_",day,"_",day,"_kappaDQF.csv",sep="")
...
checkFileExists <- function(siteName,day){
  ...
  dat <- read.csv(file=paste(siteName,"_missing_Leftover3_NDVI_Days.csv",sep=""),header=FALSE)
  for(i in 1:nrow(dat)){
...
createMissingFilesList <- function(siteName){
  diurnal.files <- dir(pattern=paste("GOES_NDVI_Leftover3",siteName,sep=""))
  days <- numeric()
  ...
  write.table(missingDays,file=paste(siteName,"_missing_Leftover3_NDVI_Days.csv",sep=""),sep=",",col.names=FALSE,row.names=FALSE)
}
...
createNDVI_GOES_LeftoverMAIN <- function(day,siteData,orbitVersion,year,TZ){
  ...
  hrs <- as.character(c(seq((1+TZ),(9+TZ),1),seq((14+TZ),(23+TZ),1))) 
  ...
  if(!dir.exists((paste("GOES_Data2017/",dir(path="GOES_Data2017",pattern=filestrACM),sep="")))){
    if(length(ACM.files>1)){
      ...
      for(i in 1:nrow(siteData)){
        siteName <- as.character(siteData[i,1])
        fileName <- paste("GOES_NDVI_Leftover3",siteName,"_",baseDay,"_",baseDay,"_kappaDQF.csv",sep="")
...
all.days <- c(seq(300,333,1),seq(348,365,1))
#all.days <- c(seq(250,333,1),seq(348,365,1))
