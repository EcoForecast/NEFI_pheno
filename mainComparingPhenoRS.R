...
for(i in iseq){
  ...
  if(PFT=="DB"){
    ...
    fileName <- paste(siteName,"_PC_varBurnINITS.RData",sep="")
    ...
    fileName <- paste(siteName,"_MODIS_varBurnINITS.RData",sep="")
    if(!file.exists(fileName)){
    ...
    if(!file.exists(fileName)){
      ...
      PC.md.out <- runMCMC_Model(j.model=j.model.PC,variableNames = SH.vars,maxIter=10**6)
      save(PC.md.out,file=fileName)
    }
    fileName <- paste(siteName,"_MODIS_varBurnINITS.RData",sep="")
    if(!file.exists(fileName)){
      j.model.MODIS <- createBayesModel.SH(dataSource="MODIS.NDVI",siteName=siteName)
      MODIS.md.out <- runMCMC_Model(j.model=j.model.MODIS,variableNames = SH.vars,maxIter=10**6)
      save(MODIS.md.out,file=fileName)
    }
    fileName <- paste(siteName,"_GOES_varBurn.RData",sep="")
    if(!file.exists(fileName)){
      j.model.GOES <- createBayesModel.SH(dataSource="GOES.NDVI",siteName=siteName)
      GOES.md.out <- runMCMC_Model(j.model=j.model.GOES,variableNames = SH.vars,maxIter=10**6)
      save(GOES.md.out,file=fileName)
    }
  }
  else if(PFT=="EN"){
    EN.vars <- c("a","b","c","d","prec")
...
#graphMCMC_Outputs_withData(outputFileName = "BayesFits_SH3.pdf",siteFileName = "GOES_Paper_Sites.csv",iseq=c(7,15),startDay=110,endDay=474)
