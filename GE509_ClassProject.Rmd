...
createBayesModel.DB <- function(Lat=0, Long=0, data.source,site.name="",URL="",niter=30000) {
  ...
  if(data.source=="PC.GCC"){
    ...
    for(i in 1:(nchain)){
      inits[[i]] <- list(a=rnorm(1,-30,5),b=rnorm(1,0.11,0.05),c=0.2,d=0.7)
    }
  }
  else if(data.source=="GOES.NDVI"){
    data = GOES_data(site.name)
    for(i in 1:(nchain-1)){
      inits[[i]] <- list(a=-30,b=rnorm(1,0.24,0.1),c=0.4,d=0.4)
...
GOES_data <- function(site.name) {
  ##Data
  NDVI.fileName <- paste("GOES_NDVI_",site.name,"2017_kappaDQF2.csv",sep="")
