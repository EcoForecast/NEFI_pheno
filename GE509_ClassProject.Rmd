...
MODIS_data <- function(Lat,Long,data.source,site.name) {
  ##Data
  fileName <- paste(site.name,"_MODIS_NDVI.csv",sep="")
  options(scipen=999)
  MODIS = read.csv(fileName,header=FALSE,as.is=TRUE,na.string="-3000")
  dat <- MODIS[,7]

  x <- as.character(MODIS[,5])
  x <- x[8:length(x)]

  y <- dat[8:length(dat)]

  for(i in 1:length(x)){
    x[i] <- as.numeric(substr(x[i],5,7))
    if(as.numeric(x[i])<100){
...
nrows <- nrows + 1
#MODIS:
out.MODIS <- createBayesModel.DB(Lat = Lat, Long=Long,data.source = "MODIS.NDVI",site.name=siteName)#,niter=500000)
...
plot(x=list(),y=list(),xlim=c(180,500),ylim=c(0,1),ylab="Value",xlab="Day of Year",main=siteName)
